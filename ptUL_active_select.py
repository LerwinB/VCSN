...
from torchvision.models import deeplabv
import umap
# set seeds
...
input_tensor_permuted = input_tensor.permute(0, 2, 3, 1)
pooling_layer = torch.nn.AdaptiveAvgPool2d((3, 256))
pooled_tensor = pooling_layer(input_tensor_permuted)

# 再次调整形状以将通道维放回正确的位置
output_tensor = pooled_tensor.permute(0, 3, 1, 2)  # 最终形状为 (batch, 256, 3, 3)
#reduce_fea=reducer.fit_transform(features.reshape(features.shape[0], -1))
print("clustering")
reduced_fea=output_tensor.numpy()
selected=None
for j in [5,10,15,20]:
    ...
    with open(os.path.join(args.data_path, f"uesatL_resnet{j}_kmeans_selected.txt"), "w") as file:
        # 遍历列表，写入每个字符串
        for i in selected:
            file.write(img_paths[i])

    with open(os.path.join("data/UESAT_RGB_53/MMdata",  f"resnet{j}_kmeans.txt"), "w") as file:
        # 遍历列表，写入每个字符串
        for i in selected:
