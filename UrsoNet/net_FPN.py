...
class UrsoNet():
    ...
    def build(self, mode, config):
        ...
        C6 = KL.Conv2D(config.BOTTLENECK_WIDTH, (3, 3), padding='SAME', strides=(2, 2), name='bottleneck_layer1')(C5)
        P4 = KL.Add(name="fpn_p4add")([
            KL.UpSampling2D(size=(4, 4), name="fpn_p5upsampled")(C6),
            KL.Conv2D(config.BOTTLENECK_WIDTH, (1, 1), name='fpnc4')(C4)])
        P3 = KL.Add(name="fpn_p3add")([
            KL.UpSampling2D(size=(2, 2), name="fpn_p4upsampled")(P4),
            KL.Conv2D(config.BOTTLENECK_WIDTH, (1, 1), name='fpnc3')(C3)])
        P2 = KL.Add(name="fpn_p2add")([
            KL.UpSampling2D(size=(2, 2), name="fpn_p3upsampled")(P3),
            KL.Conv2D(config.BOTTLENECK_WIDTH, (1, 1), name='fpnc2')(C2)])
        # Attach 3x3 conv to all P layers to get the final feature maps.
        P2 = KL.Conv2D(config.BOTTLENECK_WIDTH, (3, 3), padding='SAME', strides=(2, 2), name="fpnp2")(P2)
        #C8 = KL.Conv2D(config.BOTTLENECK_WIDTH, (2, 2), padding='SAME', strides=(1, 1), name='bottleneck_layer12345')(C7)
        ...
        print(config.IMAGE_SHAPE[0])  #第二维的数量即列数。
        nr_features = int(config.BOTTLENECK_WIDTH * config.IMAGE_SHAPE[0] * config.IMAGE_SHAPE[1] / (8 ** 2))
        # 顶层姿态和方向的输出
        loc_pred = build_loc_graph(P2, config, nr_features)
        ori_pred = build_ori_graph(P2, config, nr_features)
