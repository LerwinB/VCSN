...
def evaluate_image(model, dataset, image_id):
    ...
    I, I_meta, loc_encoded_gt, ori_encoded_gt = \
        net_FPN.load_image_gt(dataset, model.config, image_id)
...
if __name__ == '__main__':
    ...
    # Create model
    if args.command == "train":
        model = net_FPN.UrsoNet(mode="training", config=config,
                             model_dir=args.logs)
    else:
        model = net_FPN.UrsoNet(mode="inference", config=config,
                             model_dir=args.logs)
