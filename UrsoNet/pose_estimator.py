...
def evaluate(model, dataset):
    ...
    for image_id in dataset.image_ids:
        ...
        Qm.append([q_est[0],q_est[1],q_est[2],q_est[3]])
        # 4. Loc error
        loc_err = np.linalg.norm(loc_est - loc_gt)
        Tm.append([-loc_est[0], loc_est[2], -loc_est[1]])
...
def detect_dataset(model, dataset, nr_images):
    ...
    for i in range(nr_images):
        ...
        #K = np.matrix([[fx, width_ori / 2, 0], [0, height_ori / 2, fy], [0, 1, 0]])
        ...
        utils.visualize_axes(ax_1, q_gt, loc_gt, K, 200)
        utils.visualize_axes(ax_1, q_est, loc_est, K, 200)
        ax_1.plot([0, 1, 2], [0, 1, 0], 'g', label='Ground Truth')
        ax_1.plot([0, 1, 2], [0, 0.5, 1], 'r', label='Predicition')
...
def train(model, dataset_train, dataset_val):
    ...
    model.config.STEPS_PER_EPOCH = int(len(dataset_train.image_ids)/model.config.BATCH_SIZE)
...
if __name__ == '__main__':
    ...
    parser.add_argument("command", metavar="<command>", default='train',help="'train' or 'evaluate'")
    # parser.add_argument('--command', required=False, default='train', help='Backbone architecture')
    parser.add_argument('--backbone', required=False, default='resnet101', help='Backbone architecture')
    parser.add_argument('--dataset', required=False, default='/app/MMdata',help='Dataset name')
    parser.add_argument('--datalist', required=False,default='sam5_VAE3_kmeans_pose.txt', help='Dataset list txt')
    ...
    parser.add_argument('--weights', required=False,default='coco', help="Path to weights .h5 file or 'coco' or 'imagenet' for coco pre-trained weights")
    ...
    if args.command == "train":
        ...
        if args.dataset != "speed":
            dataset_train = urso.Uesat()
            dataset_train.load_dataset(dataset_dir, model.config,subset=datalist,train=True)
            dataset_val = urso.Uesat()
            dataset_val.load_dataset(dataset_dir, model.config, "val",train=False)
        else:
        ...
        if args.video:
            dataset = urso.Urso()
            dataset.load_dataset(dataset_dir, config, "train")
            detect_video(model, dataset, args.video)
        else:
            # Load validation dataset
            if args.dataset != "speed":
                dataset = urso.Uesat()
                dataset.load_dataset(dataset_dir, model.config,subset=datalist,train=False)
            else:
                dataset = speed.Speed()
        ...
        # Test dataset
        if args.dataset != "speed":
            dataset_test = urso.Uesat()
            dataset_test.load_dataset(dataset_dir, model.config,subset=datalist,train=False)
        else:
