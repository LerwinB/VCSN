...
def visualize_axes(ax, q, C, K, scale,color):
    ...
    ax.arrow(c[0, 0], c[1, 0], v[0, 0], v[1, 0], head_width=10, color=color)
    ax.arrow(c[0, 0], c[1, 0], v[0, 1], v[1, 1], head_width=10, color=color)
    ax.arrow(c[0, 0], c[1, 0], v[0, 2], v[1, 2], head_width=10, color=color)


def visualize_axes1(ax, q, C, K, scale):
    """ Shows an object pose by overlapping the object axes on a matplotlib input figure
    Arguments:
        - ax: Axis returned by a matplotlib subplot
        - q: Quaternion
