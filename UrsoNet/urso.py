...
class Camera:
    fov_x = 0.08 * np.pi / 180
    fov_y = 0.08 * np.pi / 180
...
class Urso(Dataset):
    ...
    def load_dataset(self, dataset_dir, config, subset):
        ...
        if not config.REGRESS_ORI:
            ...
            #print("ori_histogram_map is", ori_histogram_map)
            #ori_his = np.array(ori_histogram_map)
            #print("row is", ori_his.shape[0])
...
class Uesat(Dataset):
    ...
    def load_dataset(self, dataset_dir, config, subset,train=True):
        """Load a subset of the dataset.
        dataset_dir: The root directory of the dataset.
        subset: What to load (train, val, test)
        """
        ...
        # load image paths
        if train:
            with open(os.path.join(dataset_dir,subset)) as f:
                files = f.readlines()
                rgb_list = [os.path.join('images/train',file.strip('\n')+'.png') for file in files]
            poses = pd.read_csv(os.path.join(dataset_dir, subset.rstrip('.txt')+'_gt.csv'))
                
        else:
            subset1='Uesat_minival_pose.txt'
            with open(os.path.join(dataset_dir,subset1)) as f:
                files = f.readlines()
                rgb_list = [os.path.join('images/minival',file.strip('\n')+'.png') for file in files]
            poses = pd.read_csv(os.path.join(dataset_dir, subset1.rstrip('.txt')+'_gt.csv'))
        # set_filename = dataset_dir + '/' + subset + '_images.csv'
        # rgb_list_df = pd.read_csv(set_filename, names=['filename'], header=None)
        # rgb_list = list(rgb_list_df['filename'])
        ...
        print('Loading poses')
        # poses = pd.read_csv(os.path.join(dataset_dir, subset.rstrip('.txt')+'_pose_gt.csv'))
