...
def build_loc_graph(feature_map, config, nr_features):
    ...
    print("czl", nr_features)
    print("czl", np.shape(nr_features))
    x = KL.Reshape((nr_features,))(feature_map)
    print("czl", np.shape(x))
    ...
    while True:
        try:
            ...
            if b >= batch_size:
                ...
                outputs = []
                # suspension function waiting "next" to execution
    ...
    def set_log_dir(self, model_path=None):
        ...
        if model_path:
            ...
            self.log_dir = os.path.join(self.model_dir, "{}{:%Y%m%dT%H%M}".format(
                self.config.DATALIST.lower().strip('.txt'), now))

        # Path to save after each epoch. Include placeholders that get filled by Keras.
        self.checkpoint_path = os.path.join(self.log_dir, "weights_*epoch*.h5")
