...
class UEsatDataset(BaseDetDataset):
    """Dataset for COCO."""
    ...
    def filter_data(self) -> List[dict]:
        ...
        if self.split is not None:
            with open(osp.join(self.data_root,self.split),'r') as f:
                files=f.readlines()
            file_id=[file.strip('.png\n') for file in files]
            selected=[d for d in self.data_list if osp.basename(d["img_path"]).strip('.png\n') in file_id]
