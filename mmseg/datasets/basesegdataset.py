...
class BaseCDDataset(BaseDataset):
    ...
    def load_data_list(self) -> List[dict]:
        ...
        if self.split and osp.exists(osp.join(self.data_root,self.split)):
            splitfile=osp.join(self.data_root,self.split)
            # 如果提供了分割文件并且文件存在
            with open(splitfile, 'r') as file:
                # lines = [line.strip() for line in file if line.strip()]
                lines = file.readlines()
        else:
            # 如果没有提供分割文件，列出所有文件
            lines = [filename[:-len(self.img_suffix)]
                     for filename in os.listdir(self.data_prefix.get('img_path', ''))
                     if filename.endswith(self.img_suffix)]

        # 生成数据列表
        for line in lines:
            img_path = osp.join(self.data_prefix.get('img_path', ''), line.rstrip('\n') + self.img_suffix)
            seg_map_path = osp.join(self.data_prefix.get('seg_map_path', ''), line.rstrip('\n') + self.seg_map_suffix)
            data_info = {
                'img_path': img_path,
